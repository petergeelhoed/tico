...
/** @brief Frees the memory allocated for the arrays in a myarr struct.
 * @param arr A pointer to the myarr struct whose arrays are to be freed. */
void freemyarr(struct myarr* arr);

/** @brief Creates a myarr struct with an array of integers of the specified
 * length.
...
/** @brief Creates a myarr struct with an array of doubles of the specified
 * length.
