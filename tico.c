...
int main(int argc, char** argv)
{
    ...
    if (DEBUG != 0)
        fprintf(stderr,
                "Cutoff  %d < value < %d  ; n < %d ; d < %d; m = %d+-%d\n",
                cvalue,
                uvalue,
                nvalue,
                dvalue,
    ...
    { // PCM
        ...
        for (int j = 0; j < NN; j++)
        {
            in2[j][0] = .398942280401 / evalue *
                        (exp(-((float)(j * j)) / (float)(evalue * evalue) / 2) +
                         exp(-((float)(NN - j) * (NN - j)) /
                             (float)(evalue * evalue) / 2));
            // in2[j][0] = (exp(-((float)(j*j))/(float)(evalue*evalue)/2) +
        ...
        int val = 0;
        for (i = qvalue + 1;
             i <= header.overall_size / 2 + qvalue - rvalue * NN - 100;
             i++)
        {
            read = fread(lsb, sizeof(lsb), 1, stdin);
            read += fread(msb, sizeof(msb), 1, stdin);
            if (read == 2)
            {
                ...
                if (i % NN == 0)
                {
                    ...
                    if (evalue > 0)
                    {
                        ...
                        for (int j = 0; j < NN; j++)
                        {
                            conv[j][0] = (+out[j][0] * out2[j][0] -
                                          out[j][1] * out2[j][1]);
                            conv[j][1] = (out[j][0] * out2[j][1] +
                                          out[j][1] * out2[j][0]);
                        }
                    ...
                    if (wvalue && i >= lvalue * NN)
                    {
                        for (int j = 0; j < NN; j++)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    x[j - peaks[(int)(i - 1) / NN] + NN]
                                     [(int)(i - 1) / NN % pvalue] +=
                                        (float)mean[j] / max;
                                    xx[j - peaks[(int)(i - 1) / NN] + NN]
                                      [(int)(i - 1) / NN % pvalue] +=
                                        (float)mean[j] * mean[j] / max / max;
                                    nn[j - peaks[(int)(i - 1) / NN] + NN]
                                      [(int)(i - 1) / NN % pvalue] += 1;
                                }
                            }
                        }
                    ...
                    else if (tot > 0)
                    {
                        ...
                        for (j = 0; j < NN; j++)
                        {
                            if (i >= lvalue * NN && mean[j] > 0 &&
                                n <= nvalue && (abs(j - center) < dvalue) &&
                                ((mvalue > 0 && abs(j - mvalue) < svalue) ||
                                 mvalue == 0))
                            {
                                fprintf(outfile,
                                        "%ld %d %d\n",
    ...
    char command[1024];
    sprintf(command,
            " echo 'uns colorbox; f=%d;h=%d/3600; set cbtics 1; set term png "
            "size 1920,1080 font \"DejaVuSansCondensed,12 truecolor \" ; set "
            "ytics nomirror; set out \"/dev/null\"; plot \"oink\" u "
            "($1/h):($2/f); set y2tics ; set y2range "
            "[GPVAL_Y_MIN*f:GPVAL_Y_MAX*f]; set out "
            "\"/home/pi/lussen/www/tico.png\"; set fit quiet; set samples "
            "1000; d=15;d1=15; f(x)=(a-b*x+c*cos(3.1415926*(x-x0)*h/d)); "
            "g(x)=(a1-b1*x+c1*cos(3.1415926*(x-x1)*h/d1)); fit []f(x) \"oink\" "
            "u ($1/h):(int($1)%%2==0?$2/f:NaN) via a,b,c,d,x0; print "
            "d,c*1000,\"ms \",b*86400,\"s/d\"; fit []g(x) \"oink\" u "
            "($1/h):(int($1)%%2==1?$2/f:NaN) via a1,b1,c1,d1,x1; print "
            "d1,c1*1000,\"ms \",b1*86400,\"s/d\"; set xrange [:]; set key "
            "below;set samples 1000; set xlabel \"time (s)\"; set ylabel "
            "sprintf(\"modulo 1/%d s (s)\",h);plot \"oink\" u "
